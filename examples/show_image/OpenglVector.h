 ...
 * 6. draw the uploaded data via
 * `buffer.draw(static_cast<int32_t>(numberOfValuesInVertices)`
 */
class Matrix {
 public:
  Matrix(const std::vector<std::vector<float>>* const vec)
      : gpuSize_(0), vec_(vec) {}
  ...
  void draw(const int32_t numberOfValuesInVertices) const {
    ...
    for (size_t i = 0U; i < vec_->size(); ++i) {
      glDrawArrays(
          GL_TRIANGLE_STRIP, static_cast<GLint>(offset),
          static_cast<GLsizei>(static_cast<int32_t>(vec_->at(i).size()) /
                               numberOfValuesInVertices));
      offset +=
          vec_->at(i).size() / static_cast<size_t>(numberOfValuesInVertices);
    }
  }
